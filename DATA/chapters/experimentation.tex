...
\small
\begin{center}
\textbf{Table 3.} \itshape BiLSTM stage progression on 80/20 stratified split (seed 42).\\[2pt]
\upshape
...
{\scriptsize\itshape pp = absolute difference between two percentages, e.g.\ $24.7-16.1=8.6$.}\\[2pt]
{\scriptsize\color{tecdark}\textbf{Stage 03 is the documented final BiLSTM.} Two subsequent sensitivity analyses (granular parameter search) push the same architecture further -- shown shortly.}
\end{infoBox}
\end{frame}

% --- NEW: Stage progression explanation ------------------------------------
\begin{frame}[c]{Stage progression: what changed and why}
...
\textbf{Pain:} overfitting onset\\(train 66\%, val 34.6\%).
\end{minipage}
\end{block}
\end{column}
...
\small
\begin{center}
\textbf{Table 4.} \itshape Head-to-head comparison of the three temporal architectures (80/20 split, seed 42).\\[2pt]
\upshape
\begin{tabular}{l c c c}
\toprule
...
\begin{center}
\textbf{Table 5.} \itshape Sensitivity 1 -- 2$\times$2 loss/optimizer grid.\\[2pt]
\upshape
...
{\scriptsize\textbf{Figure 10.} \itshape Validation-loss overlay -- SGD descends slower but reaches a lower plateau.}
\end{column}
\end{columns}
\end{frame}
...
\setlength{\tabcolsep}{4pt}\scriptsize
\begin{center}
\textbf{Table 6.} \itshape Sensitivity 2 -- 5-point train-fraction sweep.\\[2pt]
\upshape
...
\[
\textstyle 29.9\%\;\Longrightarrow\;\textbf{36.1\%},\quad \rho\!:\;0.03\;\rightarrow\;\textbf{0.23},\quad p\!:\;0.76\;\rightarrow\;\textbf{0.001}
\]
\scriptsize\hspace*{0.5em}\textit{pp = percentage points: the absolute difference between two percentages, i.e.\ $36.1-29.9 = 6.2$.}
\end{alertblock}
\vspace{2pt}
\begin{itemize}\scriptsize
    \item If the architecture / pipeline were the bottleneck, accuracy would be \alert{roughly flat across splits} -- ours is not.
    \item The Spearman $\rho$ \alert{crosses the conventional significance threshold} ($p\!<\!0.05$) only when the test fold is large enough -- this is the textbook signature of a \alert{data-limited regime}, not a methodological flaw.
