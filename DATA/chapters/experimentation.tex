...
\begin{frame}[label=sub:evolution]{LSTM evolution: Stage 01 $\rightarrow$ Stage 02 $\rightarrow$ Stage 03}
...
\begin{block}{Stage 01 -- baseline}
\scriptsize
\textbf{Setup}
\begin{itemize}\itemsep1pt
    \item 280 clips, 5 players
    \item \alert{10-class} output (raw grades 1--10)
    \item 3 joints, 6 features
    \item No interpolation, no class weights
    \item Basic BiLSTM 128/64, dropout 0.30
\end{itemize}
\textbf{Result:} 16.1\% accuracy, macro F1 0.09\\
\textbf{Pain:} predictions collapsed to grades 6--8.
\end{block}
\end{column}

\begin{column}{0.34\textwidth}
\begin{block}{Stage 02 -- pipeline upgrades}
\scriptsize
\textbf{What was added}
\begin{itemize}\itemsep1pt
    \item Dataset grew to \alert{484 clips, 6 players}
    \item \alert{5-class} grouping (collapse adjacent grades)
    \item \alert{4 joints, 8 features} (+ left shoulder)
    \item Linear \alert{interpolation} of missing keypoints
...
\begin{column}{0.32\textwidth}
\begin{block}{Stage 03 -- regularization}
\scriptsize
\textbf{What was added}
\begin{itemize}\itemsep1pt
    \item Smaller BiLSTM \alert{64/32} (was 128/64)
    \item Dropout \alert{0.40 / 0.35}
    \item \alert{L2} weight decay ($\lambda\!=\!10^{-4}$)
    \item \alert{Gaussian-noise} augmentation ($\sigma\!=\!0.02$, $\times 2$)
    \item \alert{ReduceLROnPlateau} learning-rate scheduling
\end{itemize}
...
\begin{frame}[label=sub:comparison]{Comparison of three temporal models}
...
\begin{frame}[label=sub:sensitivity]{Sensitivity 1: Loss function $\times$ Optimizer (BiLSTM)}
\begin{columns}[T,onlytextwidth]
...
\begin{frame}[label=sub:headline]{\emph{Method is data-limited, not method-limited}}
\begin{columns}[T,onlytextwidth]
