...
% ---- Glossary page 1: Sport, vision & data ---------------------------------
\begin{frame}{Glossary -- sport, vision \& data}
\footnotesize
...
% ---- Glossary page 2: Modeling & evaluation concepts -----------------------
\begin{frame}{Glossary -- modeling \& evaluation concepts}
\footnotesize
\begin{description}\setlength{\itemsep}{3pt}
\item[\textbf{Ordinal classification}]
Supervised classification in which the target classes follow a natural ordering (\textit{Very Low} $<$ \textit{Low} $<$ \textit{Medium} $<$ \textit{High} $<$ \textit{Excellent}). Misclassifications between adjacent classes are less wrong than between distant classes, motivating rank-based metrics such as Spearman's $\rho$.

\item[\textbf{Confusion matrix}]
$K\!\times\!K$ matrix where entry $(i, j)$ records the number of samples with true class $i$ predicted as class $j$. Diagonal entries are correct classifications; off-diagonal entries reveal where the model fails.
...
\item[\textbf{Dropout}]
Regularization technique that randomly disables a fraction of neurons during each training step, forcing the network not to rely on any single feature path. This thesis uses rates of $0.40$ and $0.35$ in the two BiLSTM layers.

\item[\textbf{L2 regularization} (weight decay)]
Penalty added to the loss function proportional to the squared magnitudes of the model weights. Pushes weights toward zero, smooths decision boundaries, and helps when training data is scarce.

\item[\textbf{Overfitting}]
Phenomenon in which a model fits the training data very closely -- including its noise and player-specific idiosyncrasies -- but fails to generalize to unseen samples. Visible in this work as a $\sim$70 percentage-point gap between training and test accuracy.

\item[\textbf{Inter-rater reliability}]
Statistical measures (e.g., Cohen's $\kappa$ or the intraclass correlation coefficient) that quantify the degree of agreement between two or more annotators evaluating the same content. Not computed in this thesis because each clip was graded by a single coach -- a limitation flagged for future work.

\item[\textbf{Leave-one-player-out} (LOPO)]
Cross-validation protocol where, in each fold, all clips of one player are held out as the test set while the remaining players form the training set. Provides a conservative estimate of generalization to truly unseen individuals.
